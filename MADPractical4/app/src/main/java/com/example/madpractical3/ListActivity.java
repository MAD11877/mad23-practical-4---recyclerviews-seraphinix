...
public class ListActivity extends AppCompatActivity implements UserListOnClick{
    ...
    private List<User> generateUserList() {
        ...
        // Generate and add 20 User objects to the list with randomized data
        for (int i = 0; i < 20; i++) {
            User user = new User();
            user.setName("User " + generateRandomNumber());
            user.setDescription("Description " + generateRandomNumber());
            user.setFollowed(random.nextBoolean()); // Randomize followed state
            userList.add(user);
        }
        return userList;
    }
